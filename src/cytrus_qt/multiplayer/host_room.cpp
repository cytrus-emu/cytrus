...
#include "cytrus_qt/game_list_p.h"
#include "cytrus_qt/multiplayer/host_room.h"
#include "cytrus_qt/multiplayer/message.h"
#include "cytrus_qt/multiplayer/state.h"
#include "cytrus_qt/multiplayer/validation.h"
...
HostRoomWindow::HostRoomWindow(Core::System& system_, QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->username->text().isEmpty() && !NetSettings::values.cytrus_username.empty()) {
        // Use Cytrus Web Service user name as nickname by default
        ui->username->setText(QString::fromStdString(NetSettings::values.cytrus_username));
    }
    ...
    if (auto member = Network::GetRoomMember().lock()) {
        ...
        if (auto room = Network::GetRoom().lock()) {
            bool created = room->Create(
                ...
                ui->max_player->value(), NetSettings::values.cytrus_username,
                ...
                if (result.result_code != Common::WebResult::Code::Success) {
                    QMessageBox::warning(
                        ...
                        tr("Failed to announce the room to the public lobby. In order to host a "
                           "room publicly, you must have a valid Cytrus account configured in "
