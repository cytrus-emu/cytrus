...
#include "cytrus_qt/multiplayer/lobby.h"
#include "cytrus_qt/multiplayer/lobby_p.h"
...
Lobby::Lobby(Core::System& system_, QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->nickname->text().isEmpty() && !NetSettings::values.cytrus_username.empty()) {
        // Use Cytrus Web Service user name as nickname by default
        ui->nickname->setText(QString::fromStdString(NetSettings::values.cytrus_username));
    }
...
void Lobby::OnJoinRoom(const QModelIndex& source) {
    ...
    QFuture<void> f = QtConcurrent::run([this, nickname, ip, port, password, verify_UID] {
        std::string token;
#ifdef ENABLE_WEB_SERVICE
        if (!NetSettings::values.cytrus_username.empty() &&
            !NetSettings::values.cytrus_token.empty()) {
