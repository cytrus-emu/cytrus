 ...
 * with the native side of the Cytrus code.
 */
object NativeLibrary {
    ...
    init {
        System.loadLibrary("cytrus-android")
    }
    ...
    val isPortraitMode: Boolean
        get() = CytrusApplication.appContext.resources.configuration.orientation ==
    ...
    class EmulationErrorDialogFragment : DialogFragment() {
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val alert = MaterialAlertDialogBuilder(requireContext())
                ...
                .setMessage(
                    Html.fromHtml(
                        CytrusApplication.appContext.resources.getString(R.string.redump_games),
    ...
    /**
     * Logs the Cytrus version, Android version and, CPU.
     */
    ...
    fun createFile(directory: String, filename: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            CytrusApplication.documentsTree.createFile(directory, filename)
        } else {
    ...
    fun createDir(directory: String, directoryName: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            CytrusApplication.documentsTree.createDir(directory, directoryName)
        } else {
    ...
    fun openContentUri(path: String, openMode: String): Int =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.openContentUri(path, openMode)
        } else {
    ...
    fun getFilesName(path: String): Array<String?> =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.getFilesName(path)
        } else {
    ...
    fun getSize(path: String): Long =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.getFileSize(path)
        } else {
    ...
    fun fileExists(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.exists(path)
        } else {
    ...
    fun isDirectory(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.isDirectory(path)
        } else {
    ...
    fun copyFile(
        ...
        if (FileUtil.isNativePath(sourcePath) &&
            ...
            CytrusApplication.documentsTree
    ...
    fun renameFile(path: String, destinationFilename: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.renameFile(path, destinationFilename)
        } else {
    ...
    fun deleteDocument(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            CytrusApplication.documentsTree.deleteDocument(path)
        } else {
