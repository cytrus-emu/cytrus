...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    s_savestate_info_class = reinterpret_cast<jclass>(
        env->NewGlobalRef(env->FindClass("org/cytrus/cytrus_emu/NativeLibrary$SaveStateInfo")));
    s_core_error_class = reinterpret_cast<jclass>(
        env->NewGlobalRef(env->FindClass("org/cytrus/cytrus_emu/NativeLibrary$CoreError")));

    // Initialize NativeLibrary
    const jclass native_library_class = env->FindClass("org/cytrus/cytrus_emu/NativeLibrary");
    s_native_library_class = reinterpret_cast<jclass>(env->NewGlobalRef(native_library_class));
    s_on_core_error = env->GetStaticMethodID(
        s_native_library_class, "onCoreError",
        "(Lorg/cytrus/cytrus_emu/NativeLibrary$CoreError;Ljava/lang/String;)Z");
    ...
    // Initialize Cheat
    const jclass cheat_class = env->FindClass("org/cytrus/cytrus_emu/features/cheats/model/Cheat");
    ...
    // Initialize GameInfo
    const jclass game_info_class = env->FindClass("org/cytrus/cytrus_emu/model/GameInfo");
    ...
    s_disk_cache_progress_class = reinterpret_cast<jclass>(
        env->NewGlobalRef(env->FindClass("org/cytrus/cytrus_emu/utils/DiskShaderCacheProgress")));
    jclass load_callback_stage_class =
        env->FindClass("org/cytrus/cytrus_emu/utils/DiskShaderCacheProgress$LoadCallbackStage");
    s_disk_cache_load_progress = env->GetStaticMethodID(
        s_disk_cache_progress_class, "loadProgress",
        "(Lorg/cytrus/cytrus_emu/utils/DiskShaderCacheProgress$LoadCallbackStage;II)V");
    ...
    s_cia_install_helper_class = reinterpret_cast<jclass>(
        env->NewGlobalRef(env->FindClass("org/cytrus/cytrus_emu/utils/CiaInstallWorker")));
    ...
    jclass cia_install_status_class =
        env->FindClass("org/cytrus/cytrus_emu/NativeLibrary$InstallStatus");
