...
void InitJNI(JNIEnv* env) {
    s_software_keyboard_class = reinterpret_cast<jclass>(
        env->NewGlobalRef(env->FindClass("org/cytrus/cytrus_emu/applets/SoftwareKeyboard")));
    s_keyboard_config_class = reinterpret_cast<jclass>(env->NewGlobalRef(
        env->FindClass("org/cytrus/cytrus_emu/applets/SoftwareKeyboard$KeyboardConfig")));
    s_keyboard_data_class = reinterpret_cast<jclass>(env->NewGlobalRef(
        env->FindClass("org/cytrus/cytrus_emu/applets/SoftwareKeyboard$KeyboardData")));
    s_validation_error_class = reinterpret_cast<jclass>(env->NewGlobalRef(
        env->FindClass("org/cytrus/cytrus_emu/applets/SoftwareKeyboard$ValidationError")));

    s_swkbd_execute = env->GetStaticMethodID(
        s_software_keyboard_class, "Execute",
        "(Lorg/cytrus/cytrus_emu/applets/SoftwareKeyboard$KeyboardConfig;)Lorg/cytrus/cytrus_emu/"
...
jobject ToJavaValidationError(Frontend::ValidationError error) {
    ...
    return env->GetStaticObjectField(
        ...
        env->GetStaticFieldID(s_validation_error_class, ValidationErrorNameMap.at(error),
                              "Lorg/cytrus/cytrus_emu/applets/SoftwareKeyboard$ValidationError;"));
}

jobject Java_org_cytrus_cytrus_1emu_applets_SoftwareKeyboard_ValidateFilters(JNIEnv* env,
                                                                             jclass clazz,
                                                                             jstring text) {
...
jobject Java_org_cytrus_cytrus_1emu_applets_SoftwareKeyboard_ValidateInput(JNIEnv* env,
                                                                           jclass clazz,
                                                                           jstring text) {
