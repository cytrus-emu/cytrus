...
void CubebInput::StartSampling(const InputParameters& params) {
    ...
    auto init_result = cubeb_init(&impl->ctx, "Cytrus Input", nullptr);
    ...
    auto stream_init_result = cubeb_stream_init(
        impl->ctx, &impl->stream, "Cytrus Microphone", input_device, &input_params, nullptr,
...
std::vector<std::string> ListCubebInputDevices() {
    ...
    if (cubeb_init(&ctx, "Cytrus Input Device Enumerator", nullptr) != CUBEB_OK) {
