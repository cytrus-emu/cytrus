...
static void PrintHelp(const char* argv0) {
    std::cout << "Usage: " << argv0
              << " [options] <filename>\n"
                 ...
                 "--web-api-url       Cytrus Web API url\n"
                 "--ban-list-file     The file for storing the room ban list\n"
                 "--log-file          The file for storing the room log\n"
                 "--enable-cytrus-mods Allow Cytrus Community Moderators to moderate on your room\n"
                 "-h, --help          Display this help and exit\n"
                 "-v, --version       Output version information and exit\n";
}

static void PrintVersion() {
    std::cout << "Cytrus dedicated room " << Common::g_scm_branch << " " << Common::g_scm_desc
              << " Libnetwork: " << Network::network_version << std::endl;
}

/// The magic text at the beginning of a cytrus-room ban list file.
static constexpr char BanListMagic[] = "CytrusRoom-BanList-1";
...
int main(int argc, char** argv) {
    ...
    std::string log_file = "cytrus-room.log";
    u64 preferred_game_id = 0;
    u16 port = Network::DefaultRoomPort;
    u32 max_members = 16;
    bool enable_cytrus_mods = false;
    ...
    static struct option long_options[] = {
        ...
        {"enable-cytrus-mods", no_argument, 0, 'e'},
    ...
    if (announce) {
        if (username.empty()) {
            ...
            NetSettings::values.cytrus_username = UsernameFromDisplayToken(token);
            username = NetSettings::values.cytrus_username;
            NetSettings::values.cytrus_token = TokenFromDisplayToken(token);
        } else {
            std::cout << "Hosting a public room\n\n";
            NetSettings::values.web_api_url = web_api_url;
            NetSettings::values.cytrus_username = username;
            NetSettings::values.cytrus_token = token;
        }
    }
    if (!announce && enable_cytrus_mods) {
        enable_cytrus_mods = false;
        std::cout << "Can not enable Cytrus Moderators for private rooms\n\n";
    }
...
#else
        std::cout
            << "Cytrus Web Services is not available with this build: validation is disabled.\n\n";
