...
TEST_CASE("HLERequestContext::PopulateFromIncomingCommandBuffer", "[core][kernel]") {
    ...
    SECTION("translates StaticBuffer descriptors") {
        auto mem = std::make_shared<BufferMem>(Memory::CYTRUS_PAGE_SIZE);
    ...
    SECTION("translates mixed params") {
        auto mem_static = std::make_shared<BufferMem>(Memory::CYTRUS_PAGE_SIZE);
        MemoryRef buffer_static{mem_static};
        std::fill(buffer_static.GetPtr(), buffer_static.GetPtr() + buffer_static.GetSize(), 0xCE);

        auto mem_mapped = std::make_shared<BufferMem>(Memory::CYTRUS_PAGE_SIZE);
...
TEST_CASE("HLERequestContext::WriteToOutgoingCommandBuffer", "[core][kernel]") {
    ...
    SECTION("translates StaticBuffer descriptors") {
        std::vector<u8> input_buffer(Memory::CYTRUS_PAGE_SIZE);
        std::fill(input_buffer.begin(), input_buffer.end(), 0xAB);

        context.AddStaticBuffer(0, input_buffer);

        auto output_mem = std::make_shared<BufferMem>(Memory::CYTRUS_PAGE_SIZE);
